...
```python
import numpy as np
data = np.load("pointcloud/cloud_rgb_npz/cloud_rgb_1.npz", allow_pickle=True)
print(data.files)  # 위 키 목록 출력
```
p.load("pointcloud/cloud_rgb_npz/cloud_rgb_1.npz", allow_pickle=True)
print(data.files)  # 위 키 목록 출력
```
