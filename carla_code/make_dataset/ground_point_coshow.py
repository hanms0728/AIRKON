        ...
        try:
            while True:
                ...
                for rig in self.camera_rigs:
                    ...
                    bbs, filtered_vehicles = ClientSideBoundingBoxes.get_bounding_boxes(
                        self.vehicles, rig.camera, snapshot, MAX_DISTANCE
                    )
                    ...
                    for bb, v in zip(bbs, filtered_vehicles):
                        actor_snap = snapshot.find(v.id)
                        if actor_snap is None:
                            continue
                        veh_tf = actor_snap.get_transform()
                        offset_tf = carla.Transform(v.bounding_box.location, carla.Rotation())
                        ...
                        front_left_uv = (float(bb[3, 0]), float(bb[3, 1]))
                        front_right_uv = (float(bb[0, 0]), float(bb[0, 1]))
                        if not (self._is_point_inside_frame(front_left_uv) or self._is_point_inside_frame(front_right_uv)):
                            continue

                        annotations.append(
                            f"{CLASS_ID_CAR} {bottom_center_uv[0]:.2f} {bottom_center_uv[1]:.2f} "
