...
def main():
    parser = argparse.ArgumentParser(description="Force-set a track's yaw via the realtime fusion server command port.")
    ...
    parser.add_argument("--yaw", type=float, required=True, help="absolute yaw degrees to set")
    ...
    try:
        ...
        print(f"[set_track_yaw] {exc}", file=sys.stderr)
        sys.exit(1)
    ...
    if status == "ok":
        ...
        yaw = resp.get("yaw")
        print(f"[set_track_yaw] set track {track_id} yaw -> {yaw}")
    else:
        message = resp.get("message", "unknown error")
        print(f"[set_track_yaw] command failed: {message}", file=sys.stderr)
        sys.exit(1)
