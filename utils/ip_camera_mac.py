...
class IPCameraStreamer:
    def __init__(
        ...
        # macOS requires GUI calls on the main thread
        self.use_main_thread_visualizer = sys.platform == "darwin"
        ...
        # 시각화 스레드 (macOS에서는 메인 스레드에서 실행)
        self.vis_thread = None
        if self.use_main_thread_visualizer:
            print("[INFO] Visualizer will run on the main thread (macOS requirement).")
        else:
    ...
    def run(self):
        """Block the caller until the streamer stops."""
        try:
            if self.use_main_thread_visualizer:
                self.visualizer()
            else:
