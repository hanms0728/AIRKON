...
def main():
    ...
    # 후처리 스무딩
    ap.add_argument("--post-smooth", action="store_true", default=False,
                    help="EMA 기반 간단 트래킹/스무딩 활성화")
    ap.add_argument("--post-iou-thr", type=float, default=0.3,
                    help="트랙 매칭 IoU 임계치")
    ap.add_argument("--post-alpha", type=float, default=0.7,
                    help="EMA 알파(높을수록 최근 프레임 가중)")
    ap.add_argument("--post-max-miss", type=int, default=5,
                    help="미매칭 허용 프레임 수")
    ...
    tracker = EMATracker(
        iou_thresh=args.post_iou_thr,
        alpha=args.post_alpha,
        max_miss=args.post_max_miss
    ) if args.post_smooth else None
    ...
    for name in tqdm(names, desc="[Infer-ONNX]"):
        ...
        if tracker is not None and len(dets) > 0:
            ...
            for d in dets:
                tri = np.asarray(d["tri"], dtype=np.float32)
                tri_norm, _ = normalize_triangle(tri)
                d_new = dict(d)
                d_new["tri"] = tri_norm
                d_new["poly4"] = parallelogram_from_triangle(tri_norm[0], tri_norm[1], tri_norm[2])
                dets_norm.append(d_new)
            dets = tracker.update(dets_norm)
