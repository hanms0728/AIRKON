...
def load_camera_config_file(path: str, default_hw: Tuple[int, int]) -> List[Dict]:
    ...
    for item in entries:
        ...
        cfg = {
            ...
            # "lut_path": str((lut_rel).resolve()) if lut_rel else None,
            # "undistort_path": str((undist_rel).resolve()) if undist_rel else None,
            # "visible_ply": str((visible_ply_rel).resolve()) if visible_ply_rel else None,
    ...
    for cfg in camera_cfgs:
        ...
        if cfg.get("visible_ply"):
            ...
            if visible is not None:
                visible_source = cfg["visible_ply"]
...
def main():
    ...
    if args.viewer3d:
        for cid in cam_ids:
            ...
            try:
                bg_ply = asset.visible_source if (asset and asset.visible_source and os.path.exists(asset.visible_source)) else args.global_ply
